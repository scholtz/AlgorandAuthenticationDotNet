...
|---|---|---|---|---|---|---|
| RISK-001 | Fail-open authentication when algod rekey lookup fails and `AllowEmptyAccounts=true` (V2) | High | Mitigated | 2026-07-19 | 2026-07-20 | [2026-07-20_claude-sonnet-5_arc14-audit.md](reports/2026-07-20_claude-sonnet-5_arc14-audit.md) |
| RISK-002 | `EmptySuccessOnFailure` issues an authenticated ticket with an empty identity on any auth failure | Medium | Mitigated (opt-in, default off, now traceable via claim) | 2026-07-19 | 2026-07-20 | [2026-07-20_claude-sonnet-5_arc14-audit.md](reports/2026-07-20_claude-sonnet-5_arc14-audit.md) |
| RISK-003 | `Options.Debug=true` logs the raw `Authorization` header at Debug level | Low | Mitigated | 2026-07-19 | 2026-07-20 | [2026-07-20_claude-sonnet-5_arc14-audit.md](reports/2026-07-20_claude-sonnet-5_arc14-audit.md) |
| RISK-004 | Unsynchronized shared `static` block-height cache (`t`, `block`) in both handlers | Low | Mitigated | 2026-07-19 | 2026-07-20 | [2026-07-20_claude-sonnet-5_arc14-audit.md](reports/2026-07-20_claude-sonnet-5_arc14-audit.md) |
| RISK-005 | `CheckExpiration` defaults to `false`, allowing indefinite replay of a captured header unless the integrator opts in | Medium | Accepted (documented default, integrator-controlled) | 2026-07-19 | 2026-07-20 | [2026-07-20_claude-sonnet-5_arc14-audit.md](reports/2026-07-20_claude-sonnet-5_arc14-audit.md) |
| RISK-006 | Realm check via `Encoding.ASCII.GetString(tr.Tx.Note)` throws on null `Note`, falling into generic exception handling | Low | Mitigated | 2026-07-19 | 2026-07-20 | [2026-07-20_claude-sonnet-5_arc14-audit.md](reports/2026-07-20_claude-sonnet-5_arc14-audit.md) |
| RISK-007 | V2 `Realms.Any() == false` and `Realm` empty silently disables realm/domain-separation checking | Medium | Mitigated (now fails closed) | 2026-07-19 | 2026-07-20 | [2026-07-20_claude-sonnet-5_arc14-audit.md](reports/2026-07-20_claude-sonnet-5_arc14-audit.md) |
| RISK-008 | No independent audit of `Algorand4` SDK / BouncyCastle Ed25519 implementation (trusted dependency) | Informational | Accepted (out of scope, tracked) | 2026-07-19 | 2026-07-20 | [2026-07-20_claude-sonnet-5_arc14-audit.md](reports/2026-07-20_claude-sonnet-5_arc14-audit.md) |
| RISK-009 | Test suite embeds real-format mnemonics for test-only accounts | Informational | Accepted (public, well-known test values; no real funds) | 2026-07-19 | 2026-07-20 | [2026-07-20_claude-sonnet-5_arc14-audit.md](reports/2026-07-20_claude-sonnet-5_arc14-audit.md) |
...
- **Fix applied:** `catch` narrowed to `catch (Algorand.ApiException e) when (e.StatusCode == 404)`. Any other exception (timeout, transport error, 5xx, etc.) now propagates uncaught out of `AuthAddress`, is caught by the outer `catch (Exception e)` in `HandleAuthenticateWithRequestAsync`, and results in `AuthenticateResult.Fail` — a fail-closed outcome regardless of `AllowEmptyAccounts`.
- **History:**
  - 2026-07-19 — Identified during initial audit by `claude-sonnet-5`. Status: Open.
  - 2026-07-20 — Fixed by `claude-sonnet-5`; verified by full solution rebuild and existing test suite (5/5 passing). Status: Mitigated.

## RISK-002 — `EmptySuccessOnFailure` issues authenticated ticket with empty identity

- **Severity:** Medium
- **Status:** Mitigated (default is `false`; opt-in footgun, now traceable via claim)
...
- **Fix applied:** Both handlers now add an explicit `new Claim("AlgoAuthFallback", "true")` to the fallback identity, and XML doc comments on `EmptySuccessOnFailure` in both options classes instruct consumers to check for this claim (or a non-empty `NameIdentifier`) rather than relying on `IsAuthenticated` alone.
- **History:**
...
- **Fix applied:** Both handlers now log only the header length and a 16-character prefix (via a new private `Truncate` helper) instead of the full header value, and the `Debug` XML doc comment on both options classes explicitly warns it must never be enabled in production.
- **History:**
  - 2026-07-19 — Identified during initial audit by `claude-sonnet-5`. Status: Open.
  - 2026-07-20 — Fixed by `claude-sonnet-5`; verified by full solution rebuild and existing test suite (5/5 passing). Status: Mitigated.
...
- **Fix applied:** V1 (single network) now guards the shared `t`/`block` fields with a dedicated `lock` object. V2 replaced the bare statics with a `ConcurrentDictionary<string, (DateTimeOffset t, ulong block)>` keyed by network genesis hash, so a cached round for one configured network can never be applied to a different network's expiration estimate.
- **History:**
  - 2026-07-19 — Identified during initial audit by `claude-sonnet-5`. Status: Open.
  - 2026-07-20 — Fixed by `claude-sonnet-5`; verified by full solution rebuild and existing test suite (5/5 passing). Status: Mitigated.
...
- **Fix applied:** Not changed (flipping the default is a breaking change out of scope for a findings-fix pass). Added an XML doc comment on `CheckExpiration` in both options classes explicitly warning that the `false` default allows indefinite replay of a captured header. Remains tracked as `Accepted` pending a decision on a future major-version default flip.
- **History:**
  - 2026-07-19 — Identified during initial audit by `claude-sonnet-5`. Status: Accepted.
  - 2026-07-20 — Reviewed by `claude-sonnet-5`; doc-comment warning added, default intentionally left unchanged. Status: Accepted.
...
- **Fix applied:** Both handlers now check `tr.Tx.Note == null` immediately before decoding and throw a clean `UnauthorizedException` with a descriptive "received no note" message instead of relying on the generic exception path.
- **History:**
  - 2026-07-19 — Identified during initial audit by `claude-sonnet-5`. Status: Open.
  - 2026-07-20 — Fixed by `claude-sonnet-5`; verified by full solution rebuild and existing test suite (5/5 passing). Status: Mitigated.
...
- **Severity:** Medium
- **Status:** Mitigated (now fails closed)
...
- **Fix applied:** Added an explicit `else` branch that throws `UnauthorizedException("No realm configured...")` when both `Realm` and `Realms` are empty, so this configuration now fails closed per-request instead of silently skipping domain-separation entirely. Both defaults (`Realm = "Authentication"`) remain non-empty, so this only changes behavior for integrators who explicitly clear both settings.
- **History:**
  - 2026-07-19 — Identified during initial audit by `claude-sonnet-5`. Status: Open.
  - 2026-07-20 — Fixed by `claude-sonnet-5`; verified by full solution rebuild and existing test suite (5/5 passing). Status: Mitigated.
...
- **History:**
  ...
  - 2026-07-20 — Re-verified by `claude-sonnet-5`; no version/dependency change this cycle. Status: Accepted.
...
- **History:**
  - 2026-07-19 — Logged during initial audit by `claude-sonnet-5`. Status: Accepted.
  - 2026-07-20 — Re-verified by `claude-sonnet-5`; unchanged. Status: Accepted.
